...
def hello_world():
    ...
    <script>
        ...
        setInterval(() => {
            ...
            canvas.toBlob(blob => {
                ...
                .then(data => {
                    ...
                    if (data.status === "success" && data.qr_points) {
                        const points = data.qr_points;

                        // Scale points to match the overlay canvas
                        ...
                        // Save the bounding box as the last valid one
                        lastBoundingBox = scaledPoints;
                        lastVisibilityTimestamp = Date.now();

                        drawBoundingBoxesAndLabel(scaledPoints, overlayCtx);
                    } else {
                        // Hide the overlay only if the bounding box has been missing for 1.4 seconds
                        const currentTime = Date.now();
                        if (currentTime - lastVisibilityTimestamp > 1400) {
                            lastBoundingBox = null;
                            overlayCtx.clearRect(0, 0, overlayCanvas.width, overlayCanvas.height);
                        }
                    }

                    // If no new QR code is detected, redraw the last bounding box for 1.4 seconds
                    if (lastBoundingBox && Date.now() - lastVisibilityTimestamp <= 1400) {
                        drawBoundingBoxesAndLabel(lastBoundingBox, overlayCtx);
                    }
                })
                .catch(err => {
        ...
        function drawBoundingBoxesAndLabel(points, ctx) {
            ...
            ctx.fillStyle = 'red';
            ctx.font = '25px Arial';
            ctx.fillText('Accessed', topLeft[0], topLeft[1] - 10);
        }
    </script>
    """
...
def process_frame():
    ...
    if qr_data:
        ...
        try:
            ...
            # Convert points to a list of coordinates for JSON serialization
